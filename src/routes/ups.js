...
function logAsync(fn) {
  ...
  fn().catch(err => console.error('[db-log]', err.message));
}
...
router.post('/rate', async (req, res, next) => {
  try {
    ...
    logAsync(() => rateQuotesDb.insertQuote({
      quoteType: 'rate',
      bubbleOrderId: req.body.orderId || null,
      shipFromPostal: shipFromPostalCode,
      shipFromState: shipFromStateCode,
      shipToPostal: shipToPostalCode,
      shipToState: shipToStateCode,
      filterCount: 1,
      boxCount: 1,
      serviceCode,
      totalCharges: parseFloat(rateResponse.totalCharges) || null,
      currency: rateResponse.currency,
      requestSummary: { weight, length, width, height },
      responseSummary: { totalCharges: rateResponse.totalCharges, billingWeight: rateResponse.billingWeight },
    }));
...
router.post('/shop', async (req, res, next) => {
  try {
    ...
    logAsync(() => rateQuotesDb.insertQuote({
      ...
      filterCount: 1,
      boxCount: 1,
      serviceCode: cheapest?.serviceCode || null,
      totalCharges: cheapest ? parseFloat(cheapest.totalCharges) : null,
      currency: cheapest?.currency,
      requestSummary: { weight, length, width, height },
      responseSummary: { rateCount: rates.length, rates: rates.map(r => ({ service: r.service, cost: r.totalCharges })) },
    }));
...
router.post('/quote', async (req, res, next) => {
  try {
    ...
    const groundTotal = serviceTotals['Ground'];
    logAsync(() => rateQuotesDb.insertQuote({
      quoteType: 'quote',
      bubbleOrderId: req.body.orderId || null,
      shipFromPostal: shipFromPostalCode,
      shipFromState: shipFromStateCode,
      shipToPostal: results[0]?.address?.postalCode || null,
      shipToState: results[0]?.address?.state || null,
      filterCount: sizeList.length,
      boxCount: results.reduce((sum, r) => sum + (r.boxCount || 0), 0),
      serviceCode: groundTotal?.serviceCode || '03',
      totalCharges: groundTotal?.cost || null,
      currency: 'USD',
      requestSummary: { addressCount: results.length, filterCount: sizeList.length },
      responseSummary: { totalsByService: serviceTotals },
    }));
  ...
  try {
    ...
    for (const addressRaw of addressKeys) {
      ...
      for (const box of boxesToShip) {
        ...
        const trackingNum = firstPackage?.TrackingNumber;
        const chargeCurrency = shipmentResponse.ShipmentCharges?.TotalCharges?.CurrencyCode || 'USD';
        ...
        // Log shipment to database
        if (trackingNum) {
          logAsync(() => shipmentsDb.insertShipment({
            trackingNumber: trackingNum,
            serviceCode,
            serviceName: SERVICE_CODES[serviceCode] || serviceCode,
            bubbleOrderId: req.body.orderId || null,
            shipToName,
            shipToPhone,
            shipToAddress: parsedAddress.street,
            shipToCity: parsedAddress.city,
            shipToState: parsedAddress.state,
            shipToPostalCode: parsedAddress.postalCode,
            shipToCountryCode,
            shipFromPostalCode,
  ...
  try {
    ...
    for (const tn of numbersToVoid) {
      try {
        ...
        // Log void and update shipment status
        logAsync(async () => {
          const shipment = await shipmentsDb.getByTrackingNumber(tn);
          await voidsDb.insertVoid({
            shipmentId: shipment?.id || null,
            trackingNumber: tn,
            success: true,
            reason: req.body.reason || null,
            upsResponse: voidResponse,
          });
          if (shipment) {
            await shipmentsDb.updateShipmentStatus(tn, 'voided', { voidedAt: new Date() });
          }
        });
      } catch (error) {
        failed.push({
...
/**
 * POST /api/ups/reprint
 * Reprint shipping label and receipt for an existing shipment.
 * Fetches order data from Bubble's printShipping workflow, then generates receipts.
 *
 * REQUEST FORMAT:
 * {
 *   "shippingID": "1768251601046x907810592323898600"
 * }
 *
 * RESPONSE FORMAT:
 * {
 ...
 *     }
 *   ],
 *   "bubbleData": { ... }   // Raw data returned by Bubble (includes label if available)
 * }
 */
router.post('/reprint', async (req, res, next) => {
  try {
    ...
    if (!shippingID) {
      return res.status(400).json({ error: 'shippingID is required' });
    }

    // Validate Bubble is configured
    const BUBBLE_API_URL = process.env.BUBBLE_API_URL;
    const BUBBLE_API_KEY = process.env.BUBBLE_API_KEY;

    if (!BUBBLE_API_URL || !BUBBLE_API_KEY) {
      return res.status(500).json({
        error: 'Bubble not configured. Set BUBBLE_API_URL and BUBBLE_API_KEY environment variables.'
      });
    }

    // POST to Bubble printShipping workflow
    const url = `${BUBBLE_API_URL.replace(/\/+$/, '')}/printShipping`;
    console.log(`[reprint] Fetching order data from Bubble for shippingID: ${shippingID}`);

    const bubbleResponse = await fetch(url, {
      method: 'POST',
      headers: {
    ...
    if (!bubbleResponse.ok) {
      const body = await bubbleResponse.text();
      console.error(`[reprint] Bubble printShipping failed: ${bubbleResponse.status} — ${body}`);
      return res.status(bubbleResponse.status).json({
        error: 'Failed to fetch order data from Bubble',
        statusCode: bubbleResponse.status,
        details: body
      });
    }

    const bubbleData = await bubbleResponse.json();
    console.log(`[reprint] Bubble returned data for shippingID: ${shippingID}`);

    // Extract order data — same fields as what Bubble sends to /receipt
    const {
      shipToName,
      orderDescription,
      filterIds,
    ...
    // Generate receipts if we have the required data
    const receipts = [];

    if (shipToName && filterIds) {
      // Parse orderDescription to build filter ID → description map
      const cleanText = (orderDescription || '').replace(/\[\/?(?:ul|li|ol|b|i)\]/gi, '');
      const filterMap = {};
      const filterPattern = /([^(\n]+?)\s*\((\d+x\d+)\)/g;
      let match;
      while ((match = filterPattern.exec(cleanText)) !== null) {
        filterMap[match[2]] = match[1].trim();
      }

      // Parse filterIds (;; delimited)
      const filterEntries = filterIds.split(';;').map(s => s.trim()).filter(s => s.length > 0);
